"""
This file is the application of Fourier Neural Operator with a regulariser
(based on the paper and code by Zongyi Li et al: https://arxiv.org/pdf/2010.08895.pdf) 
for 1D Target Normal Sheath Acceleration
"""
...
class SpectralConv2d_fast(nn.Module):
    ...
    def forward(self, x):
        ...
        #print('out_ft zeros', out_ft.shape)
        out_ft[:, :, :self.modes1, :self.modes2] = \
            self.compl_mul2d(x_ft[:, :, :self.modes1, :self.modes2], self.weights1)
        #print('out_ft 1', out_ft.shape)
        out_ft[:, :, -self.modes1:, :self.modes2] = \
...
for ep in range(epochs):
    ...
    for xx, yy in train_loader:
        ...
        for t in range(0, T, step):
            ...
            loss1 += myloss(im.reshape(batch_size, -1), y.reshape(batch_size, -1))
            loss2 += loss_en(torch.sum(torch.abs(im))/torch.sum(torch.abs(y)),torch.sum(torch.abs(y))/torch.sum(torch.abs(y)))
        ...
        train_l2_step += loss1.item()
        l2_full = myloss(pred.reshape(batch_size, -1), yy.reshape(batch_size, -1))
        train_l2_full += l2_full.item()

        train_l2_step_en += loss2.item()
        # l2_full_en += loss_en(pred.reshape(batch_size, -1), yy.reshape(batch_size, -1))
        l2_full_en = loss_en(torch.sum(torch.abs(pred.reshape(batch_size, -1)))/torch.sum(torch.abs(yy.reshape(batch_size, -1))) , torch.sum(torch.abs(yy.reshape(batch_size, -1)))/torch.sum(torch.abs(yy.reshape(batch_size, -1))) )
        train_l2_full_en += l2_full_en.item()
    ...
    with torch.no_grad():
        for xx, yy in test_loader:
            ...
            test_l2_step += loss1.item()
            test_l2_full += myloss(pred.reshape(batch_size, -1), yy.reshape(batch_size, -1)).item()

            test_l2_step_en += loss2.item()
            test_l2_full_en += loss_en(torch.sum(torch.abs(pred.reshape(batch_size, -1)))/torch.sum(torch.abs(yy.reshape(batch_size, -1))) , torch.sum(torch.abs(yy.reshape(batch_size, -1)))/torch.sum(torch.abs(yy.reshape(batch_size, -1))) )
            test_all_full = loss1 + alpha*loss2
    ...
    wandb.log({
        ...
        "train_l2_step_en": train_l2_step_en/ntrain / (T / step),
        "train_l2_full_en": train_l2_full_en / ntrain,
        "test_l2_full_en": test_l2_full_en/ ntest,
        "test_l2_step_en": test_l2_step_en / ntest / (T / step),
        "test_all_full": test_all_full       
        })
